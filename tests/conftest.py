...
@pytest.fixture
def parser(tmp_path):
    """Create a PythonParser instance with tmp_path as root."""
    return PythonParser(tmp_path)


@pytest.fixture
def temp_py_file(tmp_path):
    """Create a temporary Python file (content, filename) and return its path."""
    def _create(content: str = "", filename: str = "sample.py") -> Path:
        f = tmp_path / filename
...
@pytest.fixture
def code():
    """Return various Python code snippets for testing."""
    class Code:
        minimal = ""
...
def my_func(x: int, y: Optional[str] = None) -> bool:
    """My function."""
    pass
'''
        nested_class = '''
...
def docstring_only_code():
    return '"""This module has only a docstring."""'
