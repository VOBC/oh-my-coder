...
class PermissionGuard:
    ...
    def check(self, command: str) -> CheckResult:
        ...
        # 2. 配置文件黑名单
        for pattern, compiled in zip(self.rules.denied_patterns, self._denied_re, strict=True):
            if compiled.search(command):
                return CheckResult(
                    allowed=False,
        ...
        # 3. 白名单模式
        if self._allowed_re:
            for pattern, compiled in zip(self.rules.allowed_patterns, self._allowed_re, strict=True):
                if compiled.search(command):
