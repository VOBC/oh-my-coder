...
from .package import CapabilityPackage, CapabilityPackageManager, get_manager
    ...
    if verbose:
        ...
        for pkg in packages:
            table.add_row(
                pkg.name,
                pkg.version,
                pkg.description[:40] + "..." if len(pkg.description) > 40 else pkg.description,
                ", ".join(pkg.tags[:3]),
                pkg.created_at[:10],
            )
...
def apply_capability(
    ...
    dry_run: bool = typer.Option(
        False, "--dry-run", help="预览应用结果，不实际修改"
    ),
    force: bool = typer.Option(
        False, "--force", "-f", help="强制应用，不提示确认"
    ),
):
    """
    应用能力包
...
def _load_current_config(config_path: Optional[Path] = None) -> tuple[Dict[str, Any], Dict[str, Any], List[str], Dict[str, Any]]:
    """
    加载当前项目配置
